...
public class EventhandlerLogin implements IConnectorThread {
    ...
    /**
     * calls login method for controller
     * checks previously if all inputs are filled out
     */
    @FXML
    private void login() {
    ...
    /**
     * first checks if all inputs are provided and after all information are sent to server
     * if an error accoured, a message will show up
     *
     * @param event ActionEvent from button
     */
    @FXML
    ...
    /**
     * after sending information to the server and an error accoured
     * screen will be unlocked to correct inputs
     */
    ...
    public void onWorkError(Exception e) {
        Platform.runLater(() -> {
            lbl_login_error.setText(((LoginFailedException) e).getMsg());
            lbl_login_error.setDisable(false);
            lbl_login_error.setVisible(true);
        });
        unlockInput();
